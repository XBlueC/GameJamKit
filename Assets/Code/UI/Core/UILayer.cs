...
namespace Code.UI.Core
{
    ...
    public class UILayer
    {
        ...
        public UIBase OpenUI(UIConfig uiConfig, UIArgs args)
        {
            var isExisting = false;
            if (TryFindActiveUI(uiConfig.UIType, out var ui))
            {
                isExisting = true;
            }
            else
            {
                if (ui == null)
                {
                    ui = CreateNewUI(uiConfig);
                }
            }

            Debug.Log($"OpenUI {uiConfig.UIType}");
            // 挂接到层级管理
        ...
        private bool TryFindActiveUI(UIType uiType, out UIBase ui)
        {
            ui = null;
            foreach (var node in _order)
            {
                if (node.UIType != uiType)
                {
                    continue;
                }
        ...
        private UIBase CreateNewUI(UIConfig uiConfig)
        {
            // string path = uiConfig.AssetPath;
            // var prefab = AssetsManager.Instance.LoadAsset<GameObject>(path);
            var prefab = uiConfig.Prefab;
            if (prefab == null)
            {
                throw new Exception($"UI prefab not found :{uiConfig.UIType} {uiConfig.AssetPath}");
            }

            var uiObj = Object.Instantiate(prefab, Canvas.transform);
            var ui = uiObj.GetComponent<UIBase>();
            ui.Init(uiConfig.UIType);
            return ui;
        }

        public void CloseUI(UIConfig uiConfig)
        {
            var uiBase = _order.Find(x => x.UIType == uiConfig.UIType);
            if (uiBase == null)
            {
                return;
            }
            ...
            Debug.Log($"CloseUI {uiConfig.UIType}");
        }
